...
{
    public partial class FrmInBoardProGetDataUsingExcelInput : Form
    {
        public FrmInBoardProGetDataUsingExcelInput()
        {
            InitializeComponent();
        }
        ...
        private void FrmInBoardProGetDataUsingUrl_Load(object sender, EventArgs e)
        {
            ...
            // Facebooker.FacebookerLogEvents.addToLogger += new EventHandler(ScrapperLogEvents_addToLogger);
        } 
        #endregion
        ...
        private void InBoardProGetDataThread()
        {
            ...
            foreach (string[] itemArr in excelData)
            {
                try
                {
                    ...
                    try
                    {
                        if (!string.IsNullOrEmpty(itemArr[7]))
                        {
                           // objLinkedInMaster._ProxyPassword = itemArr[7];
                            objLinkedInMaster._ProxyPort = itemArr[7];
                        }
                    }
                    catch { }
                    try
                    {
                        if (!string.IsNullOrEmpty(itemArr[8]))
                        {
                            objLinkedInMaster._ProxyUsername = itemArr[8];
                        }
                    }
                    catch
                    { }
                    try
                    {
                        if (!string.IsNullOrEmpty(itemArr[9]))
                        {
                            objLinkedInMaster._ProxyPassword = itemArr[9];
                        }
                    }
                    catch
                    { }
                    try
                    {
                        ...
                        {
                            LinkedInManager.linkedInDictionaryExcelInput.Add(objLinkedInMaster._Username, objLinkedInMaster);
                        }
                        else
                        {
                            break;
